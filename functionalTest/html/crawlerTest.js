...
if (process.env.MB_AIRPLANE_MODE !== 'true' && process.env.MB_RUN_WEB_TESTS === 'true') {
    ...
    const isLocalLink = link => link.indexOf(api.url) === 0;
    const expectedStatusCode = (link, statusCode) =>
    ...
    describe('The mountebank website', function () {
        ...
        promiseIt('should have no dead links and a valid sitemap', function () {
            ...
            return crawler.create().crawl(`${api.url}/`, '').then(result => {
                ...
                const linksNotInSitemap = siteLinks.filter(link => response.body.indexOf(link) < 0);
